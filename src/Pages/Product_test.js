// last edit  17 mordad 1403 اینجا
...
const createPd = async () => {
  try {
    ...
    // مسیر به فایل فونت در پوشه‌ی public
    const customFontBytes = await fetchFont('/ZURCHK.ttf');
    ...
    const fontSize = 30;
      ...
      page.drawImage(pngImage, {  x: 30,  y: 133 , width: 1675, height: 320 , }); 
      
    
    page.drawText('Helsdcsadadasdasdlo, world!', {
      x: 50,
      y: height - 4 ,
...
function App() {
  ...
  const handleClickversion = (event) => {  if (event.shiftKey) {alert("pressed")};   setMessage(jcode); createPd();   };
